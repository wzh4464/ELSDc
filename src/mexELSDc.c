...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    /* check arguments */
    if (nrhs < 1)
        mexErrMsgTxt("usage: mexELSDc('imagename.pgm')");
    ...
    {
        mexErrMsgIdAndTxt("mexELSDc:invalidInput", "Input argument must be a char array (single \') containing the image name.");
    }
    ...
    */
    ...
    if (ell_out != NULL)
    {
        ...
        /* return ell_out(5:) */
        plhs[0] = mxCreateDoubleMatrix(ell_count, 5, mxREAL);
        ...
        for (i = 0; i < ell_count; i++)
        {
            ...
            /* return ell_out(5:) */
            out_data[i] = ell_out[i].cx;
            out_data[i + ell_count] = ell_out[i].cy;
            out_data[i + 2 * ell_count] = ell_out[i].ax;
            out_data[i + 3 * ell_count] = ell_out[i].bx;
            out_data[i + 4 * ell_count] = ell_out[i].theta;
